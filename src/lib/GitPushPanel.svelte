<script lang="ts">
  ...
  let { projectId, visible, blocked, refreshToken, onBusyChange, onPushed,
    ports = {
      ...
      push: (projectId, expected, operationId) => invoke("project_git_push", { projectId, expected, operationId }),
      ...
      sync: (projectId, expected, operationId) => invoke("project_git_sync_tracking", { projectId, expected, operationId }),
    },
  }: {
  ...
  const cancelLabel = $derived(state.phase === "syncing" ? "取消跟踪同步" : state.phase === "verifying" ? "取消远程核对" : "取消推送操作");
</script>

{#if visible}
  <section class="push-panel" aria-label="Git 推送" aria-busy={state.busy}>
    <header>
      <strong>推送</strong>
      ...
      <button type="button" disabled={blocked || state.busy} onclick={() => controller.load()}><RefreshCw size={14} />加载远程</button>
    </header>
    {#if state.targets}
      {#if !state.targets.targets.length}<p role="status">未配置远程仓库</p>
      {:else}
        <label for="git-push-target">远程目的地</label>
        ...
        <label for="git-push-branch">目标分支</label>
        ...
        <p title={state.targets.sourceOid}>源提交 <code>{state.targets.sourceOid.slice(0, 12)}</code></p>
        <button type="button" disabled={blocked || state.busy || state.selected < 0 || !state.branch.trim()}
          onclick={() => controller.push()}><Upload size={14} />推送</button>
      {/if}
    {/if}
    {#if state.busy}<p role="status">{state.cancelling ? "正在请求取消…" : state.phase === "loading" ? "正在读取远程配置…" : state.phase === "verifying" ? "正在核对远程目标…" : state.phase === "syncing" ? "正在同步跟踪引用…" : "正在推送…"}</p>{/if}
    ...
    {#if state.result}<p role={state.result.outcome === "pushed" || state.result.outcome === "upToDate" ? "status" : "alert"}>
      {state.result.detail}<code title={state.result.sourceOid}>{state.result.sourceOid.slice(0, 12)}</code>
    ...
    {#if state.attempted}
      ...
      <p>核对分支 <span>{state.attempted.targetRef}</span></p>
      <p>核对提交 <code title={state.attempted.sourceOid}>{state.attempted.sourceOid.slice(0, 12)}</code></p>
      <button type="button" disabled={blocked || state.busy} onclick={() => controller.verify()}><SearchCheck size={14} />核对远程结果</button>
      <button type="button" disabled={blocked || state.busy} onclick={() => controller.sync()}><Download size={14} />同步跟踪引用</button>
    {/if}
    {#if state.syncResult}
      <p role={state.syncResult.outcome === "synced" ? "status" : "alert"}>
        {state.syncResult.outcome === "synced" ? "本次跟踪引用已同步" : "同步结果未知，请刷新并核对本地引用"}
    ...
    {#if state.verification}
      <p role="status">
        {state.verification.outcome === "matches" ? "远程当前公布的目标指向本次推送提交" : state.verification.outcome === "missing" ? "查询未返回目标分支" : "远程当前公布的目标指向其他提交"}
