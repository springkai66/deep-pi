...
DeepPi 运行时会在 Windows 通知区域（任务栏右下角）常驻托盘图标，图标与应用图标同源。Windows 默认把新应用的托盘图标收进溢出区，不会直接平铺在任务栏上：

1. 点击任务栏右下角的「显示隐藏的图标」（^ 箭头），在弹出的面板中找到 DeepPi 图标。
2. 把图标从面板拖到任务栏上即可常驻显示；或在系统设置 → 个性化 → 任务栏 → 其他系统托盘图标中打开 DeepPi 的开关。
3. 若刚退出应用，旧图标可能滞留（Windows 已知行为），鼠标划过即消失。

托盘交互：左键单击在「显示 / 隐藏主窗口」之间切换；右键菜单提供「显示主窗口」与「退出 DeepPi」（退出前会按当前设置确认活动任务并完成清理）。
