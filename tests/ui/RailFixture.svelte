<script lang="ts">
  // 夹具：模拟窗口以核对 DSH 模式下控制柱的收起/悬停展开（展开保持原始栏宽）。
  import "../../src/app.css";
...
<div
  style="--sidebar-w:260px; --files-w:260px; --git-w:300px"
  class:dsh-mode={dshMode}
  class:rail-closed={railCollapsed}
  class="app-shell"
>
  <nav
    class="command-rail"
  ...
  >
    ...
    <button type="button" class="rail-item selected" aria-label="Pi 工作区"><span>🤖</span></button>
    <button type="button" class="rail-item" aria-label="DSH 工作区"><span>🌐</span></button>
    <div class="rail-spacer"></div>
    <button type="button" class="rail-item" aria-label="设置"><span>⚙</span></button>
  </nav>
  <header class="topbar">
